/* For the most part, this shader is a reimplementation of the perspective as
 * defined in src/graphics/perspective.c. As awkward as it is in some ways,
 * this way the voxel renderer doesn't need to recalculate the position of
 * everything and resend it to the GPU every frame. It also allows clipping to
 * occur in the normal way (something the C code doesn't handle well, since it
 * wasn't intended for working with large polygons).
 */
